...
async function findOrCreateClass(name: string, payload?: { capacity?: number; room?: string; classTeacherId?: string | null }) {
...
async function findOrCreateSubject(name: string, classId: string, teacherId?: string | null) {
...
function mapSection(record: { id: string; name: string; class: { name: string } }): Section {
  return {
    id: record.id,
    name: record.name,
    class: record.class.name,
...
export async function updateThemeSettings(input: unknown) {
  ...
  const updated = await prisma.themeSetting.update({
    where: { id: current.id },
    data: {
      mode: toThemeMode(getString(data, 'mode', { required: false, defaultValue: current.mode })),
      primaryColor: getString(data, 'primary', { required: false, defaultValue: current.primaryColor }),
      primaryForeground: getString(data, 'primaryForeground', {
        required: false,
        defaultValue: current.primaryForeground,
      }),
      secondaryColor: getString(data, 'secondary', { required: false, defaultValue: current.secondaryColor }),
      secondaryForeground: getString(data, 'secondaryForeground', {
        required: false,
        defaultValue: current.secondaryForeground,
      }),
      accentColor: getString(data, 'accent', { required: false, defaultValue: current.accentColor }),
...
export const studentService = {
  ...
  async create(input: unknown) {
    ...
    const sectionRecord = await findOrCreateSection(classRecord.id, sectionName)
    const parentName = getString(data, 'parentName')
    const parent = await findOrCreateParentByName({
      name: parentName,
      phone: getString(data, 'phone'),
      email: getString(data, 'email').replace('@school.com', '.parent@school.com'),
  ...
  async update(id: string, input: unknown) {
    ...
    const data = asRecord(input)
    const className = getString(data, 'class', { required: false, defaultValue: current.class.name })
    const classRecord = await findOrCreateClass(className)
    const sectionName = getString(data, 'section', { required: false, defaultValue: current.section?.name ?? 'A' })
    const sectionRecord = await findOrCreateSection(classRecord.id, sectionName)

    let parentId = current.parentId
    if (data.parentName || data.phone || data.parentEmail) {
      const parent = await findOrCreateParentByName({
        name: getString(data, 'parentName', { required: false, defaultValue: current.parent?.user.name ?? '' }),
        phone: getString(data, 'phone', { required: false, defaultValue: current.user.phone ?? '' }),
        email: getString(data, 'parentEmail', {
          required: false,
          defaultValue: current.parent?.user.email ?? `${current.user.email}.parent`,
        }),
        occupation: getOptionalString(data, 'occupation') ?? current.parent?.occupation ?? null,
        address: getOptionalString(data, 'address') ?? current.parent?.address ?? null,
      })
      parentId = parent.id
    }
    ...
    const record = await prisma.student.update({
      where: { id },
      data: {
        rollNumber: getString(data, 'rollNumber', { required: false, defaultValue: current.rollNumber }),
        admissionDate: getDateString(data, 'admissionDate', current.admissionDate ?? new Date()),
        status: toUserStatus(
        ...
        user: {
          update: {
            name: getString(data, 'name', { required: false, defaultValue: current.user.name }),
            email: getString(data, 'email', { required: false, defaultValue: current.user.email }),
            phone: getString(data, 'phone', { required: false, defaultValue: current.user.phone ?? '' }),
          },
        },
      },
...
export const teacherService = {
  ...
  async create(input: unknown) {
    ...
    const subjectName = getString(data, 'subject', { required: false, defaultValue: '' })
    const classRecord = await findOrCreateClass(getString(data, 'class', { required: false, defaultValue: 'General' }))
    if (subjectName) {
  ...
  async update(id: string, input: unknown) {
    ...
    await prisma.teacher.update({
      ...
      data: {
        ...
        joiningDate: getDateString(data, 'joinDate', current.joiningDate ?? new Date()),
        status: toUserStatus(
          getString(data, 'status', {
            required: false,
            defaultValue: current.status.toLowerCase(),
    ...
    const subjectName = getString(data, 'subject', { required: false, defaultValue: current.subjects[0]?.name ?? '' })
    const classRecord = await findOrCreateClass(getString(data, 'class', { required: false, defaultValue: 'General' }))
    if (subjectName) {
...
export const staffService = {
  ...
  async update(id: string, input: unknown) {
    ...
    const record = await prisma.staff.update({
      where: { id },
      data: {
        position: getString(data, 'position', { required: false, defaultValue: current.position ?? '' }),
        department: getString(data, 'department', { required: false, defaultValue: current.department ?? '' }),
        joiningDate: getDateString(data, 'joinDate', current.joiningDate ?? new Date()),
        status: toUserStatus(
          getString(data, 'status', {
            required: false,
...
export const parentService = {
  ...
  async update(id: string, input: unknown) {
    ...
    const data = asRecord(input)
    const record = await prisma.parent.update({
      where: { id },
      data: {
        occupation: getString(data, 'occupation', { required: false, defaultValue: current.occupation ?? '' }),
        address: getString(data, 'address', { required: false, defaultValue: current.address ?? '' }),
        user: {
          update: {
            name: getString(data, 'name', { required: false, defaultValue: current.user.name }),
            email: getString(data, 'email', { required: false, defaultValue: current.user.email }),
...
export const classService = {
  ...
  async create(input: unknown) {
    const data = asRecord(input)
    const teacherName = getString(data, 'classTeacher', { required: false, defaultValue: '' })
    const teacher = teacherName ? await findOrCreateTeacherByName(teacherName) : null
    const classRecord = await findOrCreateClass(getString(data, 'name'), {
      capacity: getNumber(data, 'capacity', { required: false, defaultValue: 40 }),
      room: getString(data, 'room', { required: false, defaultValue: '' }),
      classTeacherId: teacher?.id ?? null,
    })
    const section = await findOrCreateSection(classRecord.id, getString(data, 'section'))
  ...
  async update(id: string, input: unknown) {
    ...
    const classRecord = await prisma.class.update({
      ...
      data: {
        name: getString(data, 'name', { required: false, defaultValue: currentSection.class.name }),
        capacity: getNumber(data, 'capacity', { required: false, defaultValue: currentSection.class.capacity ?? 0 }),
        room: getString(data, 'room', { required: false, defaultValue: currentSection.class.room ?? '' }),
        classTeacherId: teacher?.id ?? null,
      },
    })
    ...
    const section = await prisma.section.update({
      where: { id },
      data: {
        name: getString(data, 'section', { required: false, defaultValue: currentSection.name }),
      },
    })
...
export const sectionService = {
  ...
  async create(input: unknown) {
    ...
    const section = await findOrCreateSection(classRecord.id, getString(data, 'name'))
    return {
      id: section.id,
      name: section.name,
  ...
  async update(id: string, input: unknown) {
    ...
    const record = await prisma.section.update({
      where: { id },
      data: {
        name: getString(data, 'name', { required: false, defaultValue: current.name }),
...
export const subjectService = {
  ...
  async create(input: unknown) {
    const data = asRecord(input)
    const classRecord = await findOrCreateClass(getString(data, 'class'))
    const teacherName = getString(data, 'teacher', { required: false, defaultValue: '' })
    const teacher = teacherName ? await findOrCreateTeacherByName(teacherName) : null
  ...
  async update(id: string, input: unknown) {
    ...
    const record = await prisma.subject.update({
      ...
      data: {
        name: getString(data, 'name', { required: false, defaultValue: current.name }),
        code: getString(data, 'code', { required: false, defaultValue: current.code ?? '' }),
        classId: classRecord.id,
        teacherId: teacher?.id ?? null,
      },
      include: { class: true, teacher: { include: { user: true } } },
...
export const examService = {
  ...
  async create(input: unknown) {
    ...
    const classRecord = await findOrCreateClass(getString(data, 'class'))
    const subjectName = getString(data, 'subject', { required: false, defaultValue: '' })
    const subject = subjectName ? await findOrCreateSubject(subjectName, classRecord.id) : null
  ...
  async update(id: string, input: unknown) {
    ...
    const record = await prisma.exam.update({
      where: { id },
      data: {
        title: getString(data, 'name', { required: false, defaultValue: current.title }),
        classId: classRecord.id,
        subjectId: subject?.id ?? null,
        startDate: date,
        endDate: date,
        totalMarks: getNumber(data, 'totalMarks', { required: false, defaultValue: current.totalMarks }),
      },
      include: { class: true, subject: true },
    })
...
export const resultService = {
  ...
  async create(input: unknown) {
    ...
    const record = await prisma.result.create({
      data: {
        ...
        position: getNumber(data, 'position', { required: false, defaultValue: 0 }),
  ...
  async update(id: string, input: unknown) {
    ...
    const record = await prisma.result.update({
      ...
      data: {
        totalMarks: getNumber(data, 'totalMarks', { required: false, defaultValue: current.totalMarks }),
        grade: getString(data, 'grade', { required: false, defaultValue: current.grade ?? '' }),
        gpa: getNumber(data, 'gpa', { required: false, defaultValue: current.gpa ?? 0 }),
        position: getNumber(data, 'position', { required: false, defaultValue: current.position ?? 0 }),
...
export const feeService = {
  ...
  async create(input: unknown) {
    ...
    const record = await prisma.fee.create({
      data: {
        ...
        status: toFeeStatus(getString(data, 'status', { required: false, defaultValue: 'pending' })),
        month: getString(data, 'month'),
        title: getString(data, 'title', { required: false, defaultValue: 'Monthly Fee' }),
      },
      include: { student: true },
    })
  ...
  async update(id: string, input: unknown) {
    ...
    const student = await prisma.student.findFirst({ where: { studentId: studentCode } })
    if (!student) {
      throw new ApiError(404, 'Student not found')
    }
    ...
    const record = await prisma.fee.update({
      ...
      data: {
        studentId: student.id,
        amount: getNumber(data, 'amount', { required: false, defaultValue: current.amount }),
        ...
        month: getString(data, 'month', { required: false, defaultValue: current.month }),
...
export const accountService = simpleCrud<AccountTransaction>({
  ...
  list: async () => (await prisma.accountTransaction.findMany({ orderBy: { date: 'desc' } })).map(mapAccount),
  get: async id => {
    const record = await prisma.accountTransaction.findUnique({ where: { id } })
  ...
  update: async (id, data) => {
    const current = await prisma.accountTransaction.findUnique({ where: { id } })
    if (!current) {
    ...
    const record = await prisma.accountTransaction.update({
      ...
      data: {
        ...
        type: toTransactionType(
          getString(data, 'type', { required: false, defaultValue: current.type.toLowerCase() })
        ),
        amount: getNumber(data, 'amount', { required: false, defaultValue: current.amount }),
        category: getString(data, 'category', { required: false, defaultValue: current.category }),
        date: getDateString(data, 'date', current.date),
...
export const payrollService = simpleCrud<Payroll>({
  ...
  list: async () => (await prisma.payroll.findMany({ orderBy: { createdAt: 'desc' } })).map(mapPayroll),
  get: async id => {
  ...
  create: async data => {
    const record = await prisma.payroll.create({
      data: {
        ...
        month: getString(data, 'month'),
        amount: getNumber(data, 'amount'),
        status: toPayrollStatus(getString(data, 'status', { required: false, defaultValue: 'pending' })),
  ...
  update: async (id, data) => {
    ...
    const record = await prisma.payroll.update({
      where: { id },
      data: {
        employeeName: getString(data, 'employeeName', { required: false, defaultValue: current.employeeName }),
        ...
        paidDate: getDateString(data, 'paidDate', current.paidDate ?? new Date()),
      },
    })
    return mapPayroll(record)
  },
...
export const inventoryService = simpleCrud<Inventory>({
  ...
  list: async () => (await prisma.inventory.findMany({ orderBy: { createdAt: 'desc' } })).map(mapInventory),
  get: async id => {
  ...
  update: async (id, data) => {
    ...
    const record = await prisma.inventory.update({
      ...
      data: {
        name: getString(data, 'name', { required: false, defaultValue: current.name }),
        quantity: getNumber(data, 'quantity', { required: false, defaultValue: current.quantity }),
        unit: getString(data, 'unit', { required: false, defaultValue: current.unit ?? '' }),
        price: getNumber(data, 'price', { required: false, defaultValue: current.price ?? 0 }),
        note: getString(data, 'note', { required: false, defaultValue: current.note ?? '' }),
      },
    })
    return mapInventory(record)
...
export const transportService = simpleCrud<Transport>({
  ...
  list: async () => (await prisma.transport.findMany({ orderBy: { createdAt: 'desc' } })).map(mapTransport),
  get: async id => {
  ...
  create: async data => {
    const record = await prisma.transport.create({
      data: {
        ...
        driverName: getString(data, 'driverName', { required: false, defaultValue: '' }),
        driverPhone: getString(data, 'driverPhone', { required: false, defaultValue: '' }),
  ...
  update: async (id, data) => {
    ...
    const record = await prisma.transport.update({
      where: { id },
      data: {
        routeName: getString(data, 'routeName', { required: false, defaultValue: current.routeName }),
        vehicleNo: getString(data, 'vehicleNo', { required: false, defaultValue: current.vehicleNo }),
        driverName: getString(data, 'driverName', { required: false, defaultValue: current.driverName ?? '' }),
        driverPhone: getString(data, 'driverPhone', { required: false, defaultValue: current.driverPhone ?? '' }),
        fee: getNumber(data, 'fee', { required: false, defaultValue: current.fee ?? 0 }),
      },
    })
    return mapTransport(record)
  },
...
export const homeworkService = simpleCrud<Homework>({
  ...
  create: async data => {
    const classRecord = await findOrCreateClass(getString(data, 'class'))
    const subject = await findOrCreateSubject(getString(data, 'subject'), classRecord.id)
...
export const smsService = simpleCrud<SmsLog>({
  ...
  list: async () => (await prisma.smsLog.findMany({ orderBy: { createdAt: 'desc' } })).map(mapSms),
  get: async id => {
  ...
  create: async data => {
    const record = await prisma.smsLog.create({
      data: {
        ...
        type: getString(data, 'type', { required: false, defaultValue: 'general' }),
        status: getString(data, 'status', { required: false, defaultValue: 'queued' }),
      },
    })
  ...
  update: async (id, data) => {
    ...
    const record = await prisma.smsLog.update({
      where: { id },
      data: {
        receiver: getString(data, 'receiver', { required: false, defaultValue: current.receiver }),
        message: getString(data, 'message', { required: false, defaultValue: current.message }),
        type: getString(data, 'type', { required: false, defaultValue: current.type }),
        status: getString(data, 'status', { required: false, defaultValue: current.status }),
      },
    })
    return mapSms(record)
  },
...
export const roleService = {
  ...
  async update(id: string, input: unknown) {
    ...
    const record = await prisma.role.update({
      ...
      data: {
        name: getString(data, 'name', { required: false, defaultValue: current.name }),
        description: getString(data, 'description', { required: false, defaultValue: current.description ?? '' }),
        permissions: {
          create: permissions.map(permission => ({
            permissionId: permission.id,
          })),
...
export const permissionService = simpleCrud<{
  ...
  update: async (id, data) => {
    ...
    const record = await prisma.permission.update({
      ...
      data: {
        name: getString(data, 'name', { required: false, defaultValue: current.name }),
        module: getString(data, 'module', { required: false, defaultValue: current.module }),
        action: getString(data, 'action', { required: false, defaultValue: current.action }),
        description: getString(data, 'description', { required: false, defaultValue: current.description ?? '' }),
      },
    })
    return {
...
export async function getDashboardStats() {
  const [totalStudents, totalTeachers, totalStaff, totalClasses, pendingFees, feeAggregate] = await Promise.all([
